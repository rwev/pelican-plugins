...
def make_page(readers, context, filename, basepath):
    basepath = os.path.abspath(basepath) # to compare with filepath

    filepath, filename = os.path.split(filename)
    page = readers.read_file(filepath, filename, Page, None, context)

    slug, _ = os.path.splitext(
        os.path.join(
            os.path.relpath(
                filepath,
                basepath
    ...
    slug = os.path.relpath(slug) # remove rel path prefixing, e.g. "./"
    return slug, page
...
def make_pages(readers, context, path):
    ...
    for filename in yield_files(path):
        try:
            slug, page = make_page(readers, context, filename, path)
        except Exception:
